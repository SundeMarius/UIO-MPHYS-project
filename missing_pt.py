...
# Debug
#labels_pt10 = [[r"LO, no jet", r"LO, min jet $p_T=10$GeV"], [r"NLO, min jet $p_T=10$GeV", r"NLO, min jet $p_T=10$GeV"]]
#labels_pt1 = [[r"LO, no jet", r"LO, min jet $p_T=1$GeV"], [r"NLO, min jet $p_T=1$GeV", r"NLO, min jet $p_T=1$GeV"]]
...
filenames_pt10 = [[path_LO_1, path_LO_2_pt10], [path_NLO_1_pt10, path_NLO_2_pt10]]
...
for p, pt in enumerate(filenames):
	...
	for f, filename in enumerate(pt):
		...
		if os.path.isfile(res_file):
		    ...
		    events, num_events = util.combine_LHE_files(filename[0], filename[1], xsec[f][0], xsec[f][1], pt_cut=20e10)
		    #events, num_events = util.combine_LHE_files(filename[0], filename[1], 0, 0, pt_cut=20)
...
LO_hist_pt10 = histograms[0]  # tuple (count, bin_edges), Q-distribution representing the model
NLO_hist_pt10 = histograms[1]  # tuple (count, bin_edges), P-distribution representing the data

LO_hist_pt1 = histograms[2]  # tuple (count, bin_edges), Q-distribution representing the model
NLO_hist_pt1 = histograms[3]  # tuple (count, bin_edges), P-distribution representing the data
...
plot.add_series(np.array([]), np.array([]), label=r"KL-div(LO$\rightarrow$NLO, $p_T=10$GeV): %1.2e bits"%kl_div_pt10, lw=0)
plot.add_series(np.array([]), np.array([]), label=r"KL-div(LO$\rightarrow$NLO, $p_T=1$GeV): %1.2e bits"%kl_div_pt1, lw=0)
